...
def get_player_score(id, name):
    ...
    with Session(autoflush=False, bind=engine) as db:
        game_name = (
            db.query(Games)
            .filter(Games.chat_id == id, Games.online_status is True)
            .first()
        )
        if game_name:
            ...
            player = (
                db.query(Player)
                .filter(Player.game_name == game_name, Player.name == name)
                .first()
            )
...
def get_id_chats(game_name):
    ...
    with Session(autoflush=False, bind=engine) as db:
        game = (
            db.query(Games)
            .filter(Games.game_name == game_name, Games.online_status is True)
            .all()
        )
        if game:
...
def all_players(chat_id):
    ...
    with Session(autoflush=False, bind=engine) as db:
        ...
        game_name = (
            db.query(Games)
            .filter(Games.chat_id == chat_id, Games.online_status is True)
            .first()
        )
        if game_name:
            game_name = game_name.game_name
...
def player_status_change(game_name: str, name: str):
    ...
    with Session(autoflush=False, bind=engine) as db:
        ...
        player = (
            db.query(Player)
            .filter(Player.name == name, Player.game_name == game_name)
            .first()
        )
        if player:
            ...
            db.commit()
            players_status = (
                db.query(Player.name, Player.play_status)
                .filter(Player.game_name == game_name)
                .all()
            )
        else:
            players_status = []
...
def all_games_online():
    ...
    with Session(autoflush=False, bind=engine) as db:
        # создаем объект Game для добавления в бд
        games = db.query(Games.game_name).filter(Games.online_status is True).all()
